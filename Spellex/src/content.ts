...
const TYPING_INTERVAL = 2000; // Wait 2 seconds after user stops typing to trigger check
...
// In-flight correction tracking
let isCheckInFlight = false;          // True while an API call is pending
let blocksBeingChecked: Set<string> = new Set(); // Block texts currently being corrected
let inFlightStale = false;            // True when the user edits a block mid-flight
let pendingTarget: HTMLElement | null = null; // Element queued for check after flight lands
...
/**
 * Runs a spell-check on the given element.
 * If a flight is already in progress, queues this element for after the flight lands.
 */
function runCheck(element: HTMLElement) {
    // If a flight is in progress, queue this element to be checked once it lands
    if (isCheckInFlight) {
    ...
    const finalizeFlight = () => {
        ...
        if (inFlightStale) {
            // User modified the block being corrected — clear cache so it re-checks fresh
            console.log('Spellex: Flight was stale. Clearing cache for re-check.');
            checkedBlocks.clear();
            inFlightStale = false;
            hideLoadingState();
        } else {
            // Normal finish — show result
        ...
        // If the user typed something new while we were in-flight, check it now
        if (pendingTarget) {
        ...
        try {
            chrome.runtime.sendMessage(
                ...
                (response) => {
                    ...
                    // Only apply corrections if this flight is still valid (not stale)
                    if (!inFlightStale) {
...
document.addEventListener('input', (event) => {
    ...
    // Always reset the debounce timer on every keystroke
    if (typingTimer) clearTimeout(typingTimer);

    // If a flight is running, check if the user is modifying the block being corrected
    if (isCheckInFlight) {
        ...
        // Schedule a debounce — runCheck() will queue if still in-flight when timer fires
        typingTimer = window.setTimeout(() => runCheck(finalTarget), TYPING_INTERVAL);
        return;
    }

    // Normal debounce — starts a check after user stops typing for TYPING_INTERVAL ms
    typingTimer = window.setTimeout(() => runCheck(finalTarget), TYPING_INTERVAL);
